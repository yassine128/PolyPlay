...
class PlaylistService {
  ...
  async getPlaylistById (id) {
    ...
    const playlist = allPlaylists.find(element => element.id === id);
  ...
  async search (substring, exact) {
    const regexOption = exact ? '' : 'i';
    const filter = {
      $or: [
        { name: { $regex: `${substring}`, $options: regexOption } },
        { description: { $regex: `${substring}`, $options: regexOption } }
      ]
    };
