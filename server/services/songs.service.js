...
class SongService {
  ...
  async getSongById (id) {
    ...
    const song = allSongs.find(element => element.id === parseInt(id));
    return song === undefined ? null : song;
  }
  ...
  async updateSongLike (id) {
    ...
    await this.collection.updateOne({ id: parseInt(id) }, setQuery);
    return liked;
  }
  ...
  async search (substring, exact) {
    const caseSensible = exact ? '' : 'i';

    const filter = {
      $or: [
        { name: { $regex: `${substring}`, $options: caseSensible } },
        { artist: { $regex: `${substring}`, $options: caseSensible } },
        { genre: { $regex: `${substring}`, $options: caseSensible } },
      ]
    };
